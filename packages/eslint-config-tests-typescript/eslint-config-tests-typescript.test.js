...
describe('<Header/>', () => {
  test('shows login link if current user is no logged in', () => {
  ...
  test('shows home link if current user is logged in', () => {
  ...
  test('shows logout button if current user is logged in', () => {
    const store = observable({ isLoggedIn: true })
